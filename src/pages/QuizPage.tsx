            ...
            {isFinished ? (
                ...
                >
                    ...
                    <div className="text-center mb-12 relative z-10">
                        ...
                        >
                            <Trophy className={`mx-auto mb-6 ${score === quiz.questions.length ? 'text-amber-400' : 'text-slate-500'}`} size={64} />
                            <h2 className="text-4xl font-black text-white mb-4 tracking-tight">Cicle Finalitzat</h2>
                            ...
                            <div className="flex items-center justify-center gap-4 mb-6">
                                ...
                                <div className="text-left">
                                    <p className="text-slate-400 text-sm uppercase tracking-widest font-bold">Puntuació</p>
                                    <p className="text-white font-mono text-xl">{score} / {quiz.questions.length}</p>
                                </div>
                            </div>

                            <p className="text-slate-300 text-lg max-w-lg mx-auto leading-relaxed">
                                {score === quiz.questions.length
                                    ? "Perfecte. Has demostrat un domini absolut de les bases de dades i algorísmia de C++. Estàs preparat per a qualsevol repte tècnic d'alt nivell."
                                    : "Analitza els teus errors."}
                            </p>
                        </motion.div>
                    </div>
                        ...
                        {quiz.questions.map((q, i) => {
                            ...
                            return (
                                <motion.div
                                    ...
                                    className={`p-6 rounded-3xl border transition-all ${isCorrect ? 'bg-emerald-500/5 border-emerald-500/20' : 'bg-red-500/5 border-red-500/20'}`}
                                >
                                    <div className="flex gap-4">
                                        <div className={`mt-1 p-1.5 rounded-full shrink-0 ${isCorrect ? 'bg-emerald-500/20 text-emerald-400' : 'bg-red-500/20 text-red-400'}`}>
                                            {isCorrect ? <CheckCircle size={20} /> : <XCircle size={20} />}
                                        </div>
                                        <div className="flex-1">
                                            <h4 className="text-slate-200 font-bold leading-relaxed mb-3">{i + 1}. {q.question}</h4>
                                            ...
                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-4">
                                                <div className={`p-3 rounded-xl text-xs font-medium border ${isCorrect ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-300' : 'bg-red-500/10 border-red-500/20 text-red-300'}`}>
                                                    ...
                                                    {q.options.find(o => o.id === userAnswer)?.text || 'No contestada'}
                                                </div>
                                                {!isCorrect && (
                ...
                // QUIZ SCREEN - Immersive Challenge
                <div className="flex flex-col flex-1">
                    ...
                    <AnimatePresence mode="wait">
                        <motion.div
                            ...
                            className="bg-slate-900/40 backdrop-blur-2xl border border-white/10 rounded-[2.5rem] p-8 md:p-12 shadow-2xl mb-8 flex-1 flex flex-col"
                        >
                            <div className="flex items-center justify-between mb-8">
                                <span className="bg-primary/10 text-primary px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-[0.2em]">
                                    Challenge {currentQuestionIdx + 1}/{quiz.questions.length}
                                </span>
                                <div className="flex gap-1.5">
                                    {['A', 'B', 'C', 'D'].map((l, i) => (
                                        <div key={l} className={`w-5 h-5 rounded border flex items-center justify-center text-[10px] font-bold ${selectedAnswers[currentQ.id] ? (i === currentQ.options.findIndex(o => o.id === selectedAnswers[currentQ.id]) ? 'bg-primary border-primary text-white' : 'border-white/10 text-white/20') : 'border-white/10 text-white/40'}`}>
                                            {l}
                                        </div>
                                    ))}
                                </div>
                            ...
                            <h2 className="text-2xl md:text-3xl text-white font-bold leading-tight mb-8">
                                {currentQ.question}
                            </h2>

                            {currentQ.codeSnippet && (
                                <div className="rounded-2xl overflow-hidden mb-8 border border-white/5 shadow-inner bg-[#0d1117]">
                                    <pre className="!m-0 !p-6 !bg-transparent">
                                        <code className="language-cpp font-mono text-sm leading-relaxed">
                                            {currentQ.codeSnippet}
                                        </code>
                                    </pre>
                                </div>
                                ...
                                {currentQ.options.map((opt, i) => {
                                    ...
                                    return (
                                        <motion.button
                                            ...
                                            whileHover={{ x: 4 }}
                                            whileTap={{ scale: 0.98 }}
                                            className={`group text-left p-5 rounded-2xl border transition-all flex gap-5 items-center relative overflow-hidden ${isSelected
                                                ? 'bg-primary/10 border-primary shadow-[0_0_30px_rgba(14,165,233,0.15)] ring-1 ring-primary/50'
                                        ...
                                        >
                                            ...
                                            <div className={`w-8 h-8 rounded-xl border flex items-center justify-center shrink-0 transition-all font-mono font-bold ${isSelected
                                                ? 'bg-primary border-primary text-white scale-110'
                                                : 'border-white/10 bg-black/20 text-slate-500 group-hover:border-white/30 group-hover:text-slate-300'
                                                }`}>
                                                {letters[i]}
                                            </div>
                                            <span className={`leading-relaxed text-[15px] md:text-base transition-colors ${isSelected ? 'text-white font-semibold' : 'text-slate-300'}`}>
                                                {opt.text}
                                            </span>
                                        </motion.button>
                                    );
                    ...
                    <div className="flex items-center justify-between gap-4">
                        <button
                            ...
                            className="flex items-center gap-2 px-6 py-4 rounded-2xl border border-white/10 bg-slate-900/50 hover:bg-slate-800 text-slate-400 hover:text-white disabled:opacity-0 disabled:pointer-events-none transition-all font-bold"
                        >
                            <ChevronLeft size={20} /> <span className="hidden sm:inline">Anterior</span>
                        </button>

                        <div className="flex-1 max-w-xs h-px bg-white/5 hidden md:block" />

                        <button
                            onClick={handleNext}
                            disabled={!selectedAnswers[currentQ.id]}
                            className={`flex items-center justify-center gap-2 px-10 py-4 rounded-2xl font-black uppercase tracking-widest text-sm transition-all relative overflow-hidden ${!!selectedAnswers[currentQ.id]
                                ? 'bg-white text-slate-950 shadow-2xl shadow-white/10 hover:scale-105 active:scale-95'
                                : 'bg-white/5 text-white/20 border border-white/5 cursor-not-allowed'
                                }`}
                        >
                            <span>{currentQuestionIdx === quiz.questions.length - 1 ? 'Avaluar' : 'Següent'}</span>
                            {currentQuestionIdx !== quiz.questions.length - 1 && <ChevronRight size={18} />}

                            {/* Keyboard hint hint */}
                            {!!selectedAnswers[currentQ.id] && (
                                <span className="absolute bottom-1 right-2 text-[8px] font-bold opacity-30 hidden sm:block">↵ ENTER</span>
                            )}
                        </button>
                    </div>
                </div>
