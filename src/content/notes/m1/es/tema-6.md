...
description: "Introducción a los espacios vectoriales, subespacios, bases y dimensión."
order: 7
readTime: "25 min"
subject: "m1"
draft: false
...
Sean $x = (x_1, \dots, x_n)$ e $y = (y_1, \dots, y_n)$ dos elementos de $\mathbb{R}^n$, y sea $\lambda \in \mathbb{R}$ un escalar:
...
Es un universo dentro de otro universo que conserva la misma "física" (las operaciones de suma y producto). Este subconjunto es **autónomo**: si te limitas a operar con sus vectores haciendo **combinaciones lineales**, nunca podrás escapar de él.

Para que esto sea posible, hay dos requisitos:
1.  **El origen debe estar**: No puedes tener un universo sin un centro de coordenadas $(0,0,0)$.
...
Formalmente, un subconjunto no vacío $S \subseteq E$ es un **subespacio vectorial** si él mismo tiene estructura de espacio vectorial con las mismas operaciones que $E$. En la práctica, solo hace falta verificar que es **cerrado por combinaciones lineales**:
...
*   Si el sistema es **Compatible Determinado (SCD)** $\implies$ la única solución es el cero $\implies$ **LI**.
...
La intersección es el conjunto de vectores que **pertenecen a los dos universos a la vez**.
...
### Base
Un conjunto de vectores $B = \{u_1,\dots,u_n\}$ se dice que es una base de un SEV $S$ si y solo si rango $P^B_C = n = \dim(S)$:

1. **Linealmente independiente (LI)**.
2. **Generador** de $S$.
...
Esto quiere decir que si conoces las coordenadas en base $B$, puedes encontrar el vector "normal" (canónico) haciendo: $v_{can} = P_{can}^B \cdot v_B$.
:::

<!-- 
---

## 7. Guía práctica: Ecuaciones de SEV

### A. Ecuaciones paramétricas
Expresan cada componente del vector en función de unos parámetros ($\alpha, \beta, \gamma \dots$).
- **De generadores a paramétricas**: Simplemente escribe la combinación lineal genérica.
...
*Número de parámetros = $\dim E - \text{número de ecuaciones LI}$.*

:::tip Recordatorio para el Examen
Para ampliar un conjunto LI hasta ser una base de $E$, simplemente añade vectores de la base canónica de uno en uno, comprobando que el rango aumente, hasta llegar a $\dim E$ vectores.
::: -->
