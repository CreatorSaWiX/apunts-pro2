...
- Si **multiplicamos** por un número **negativo** ($c < 0$), la desigualdad se gira: $a < b \implies ac > bc$.
...
¿Cómo medimos la distancia entre dos puntos? No nos importa si vamos de derecha a izquierda o al revés, la distancia tiene que ser positiva. Definimos el valor absoluto $|x|$:
$$
|x| = \begin{cases}
  x  & \text{si } x \geq 0 \\
  -x & \text{si } x < 0
...
**Propiedades:**
- **Desigualdad triangular:** Ir directo siempre es más corto o igual que hacer escala: $|x+y| \leq |x| + |y|$
- **Entornos:** $|x| < a$ equivale a decir que $x$ está atrapado entre $-a$ y $a$: $-a < x < a$.

## 1.4 Cotas: Supremo, ínfimo, máximo y mínimo
Imaginemos un conjunto $A$ dentro de la recta real:

- **Cota superior:** Un número $k$ es cota superior si es mayor o igual que TODOS los elementos del conjunto. Es como un 'techo'. Si lo tiene, el conjunto está acotado superiormente.
...
**Ejemplo:** Consideremos el intervalo $A = [0, 2)$.
...
**Teorema del extremo:** En $\mathbb{R}$, todo conjunto no vacío y acotado superiormente siempre tiene supremo. Esta propiedad no pasa en los racionales (donde podría haber un agujero justo donde iría el supremo).
...
Esto quiere decir que $a$ es una raíz (solución de $f(x) = 0$) si y solo si la división es exacta. En los reales, cualquier polinomio se puede descomponer en factores de grado 1 (tipo $x - a$) o de grado 2.
