...
Antes de ver hacia dónde viaja una sucesión, tenemos que saber si está "encerrada" o si se va hacia el infinito.

**Cota superior:** Si podemos encontrar un número real $k$ que sea mayor o igual que todos los términos de la sucesión ($a_n \leq k$), decimos que es una cota superior. De todas las cotas superiores posibles, a la más pequeña se la llama **supremo**.

**Cota inferior:** De la misma manera, si un número $k$ siempre se queda por debajo de los términos ($k \leq a_n$), es una cota inferior. La mayor de las cotas inferiores se llama **ínfimo**.
...
Si continuamos la lista infinitamente, ¿nos acercamos a algún número concreto? Eso es el **límite**:
...
> 
> Si la variable está en base y el exponente es constante como $n^\alpha$:
...
Ahora bien, hay "choques de infinitos" donde la respuesta no es evidente llamadas **indeterminaciones**. Tenemos que vigilar especialmente con estos casos:

$$
\frac{\infty}{\infty} \quad \quad \frac{0}{0} \quad \quad \infty - \infty \quad \quad 1^\infty, 0^0, y\ \infty^0
$$

Para los polinomios: si tenemos $\frac{\infty}{\infty}$ formado por polinomios $\frac{P(n)}{Q(n)}$, fijémonos en los grados.
...
El **criterio raíz-cociente** nos avisa de que si el criterio del cociente da un resultado $L$, el de la raíz dará exactamente el mismo $L$. Sin embargo, tened cuidado: podría darse el caso de que el límite de la raíz exista pero el del cociente no se pueda calcular.
...
> Por el contrario, en $\frac{n^2 - 6n - 2}{3n^2 - 9n}$, los grados son idénticos. Entonces se evalúa la división coeficiente de arriba contra el del denominador, tenemos $\frac{1}{3}$.
...
>  
> Cuando hay sumatorios masivos hará falta sándwich:
>  Analizando $\frac{1}{\sqrt{n^2 + 1}} + \frac{1}{\sqrt{n^2 + 2}} + \dots + \frac{1}{\sqrt{n^2 + n}}$, construimos acotando a nivel de raíces:
>  Límite superior $n \cdot \frac{1}{\sqrt{n^2 + 1}}$ y Límite Inferior $n \cdot \frac{1}{\sqrt{n^2 + n}}$. Como ambos desembocan de manera de grados igual a $1$, la respuesta a sucesión en medio atrapada del sándwich será un $1$.
>
> También aplica lo rápido y lento que hemos visto sobre las **jerarquías**: En  $\lim_{n \rightarrow +\infty} \frac{a^n}{n!}$ siendo $a=10$, para n grandes el factorial de n aplasta exponencial y lo hace 0. Todo va determinado bajo $\frac{\text{Lento}}{\text{Rápido}} \rightarrow 0$.
>
> Y referente a trigonométricas, como en $\frac{\cos n}{n^2}$, por el hecho de que $\cos$ estará comprendido y acotado por valores entre -1 y 1, acaba dividiendo por algo enorme como $n^2$, entonces forzosamente se orienta hacia $0$. Y para indeterminación de exponenciales $\frac{2^n + 3^n}{2^n - 3^n}$ cogemos quién lidera: Al elevar con elemento predominante encontraremos fracciones con elevados a n que van a 0. Siendo la resolución final a $-1$. Este sería el caso equivalente como las constantes $\left(\frac{n+2}{n-3}\right)^{\frac{2n-1}{5}}$ que hay que aplicar regla de Euler como $e^{b_n \cdot (a_n - 1)}$ con resultado de $e^2$.
> Resta de raíces se solventa como lo explicado por el conjugado: $(\sqrt{n+1} - \sqrt{n})\sqrt{\frac{n+1}{2}}$, que como método nos lleva a $\frac{\sqrt{2}}{4}$. Y problemas recurrentes donde pone  ej. $a_{n+1} = ...$ piden escribir la iteración del límite bajo relación encontrada.
