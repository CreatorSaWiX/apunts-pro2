...
Derivem el numerador $N(x) = \\int_{0}^{x^2} \\sin \\sqrt{t} \\, dt$:
$N'(x) = \\sin(\\sqrt{x^2}) \\cdot \\frac{d}{dx}(x^2) = \\sin(x) \\cdot 2x = 2x \\sin x$
*(Nota: Com que $x \\to 0^+$, $\\sqrt{x^2} = x$)*
...
A prop de $x=0$, $e^{x^2} \\approx 1$ i $\\sin x \\approx x$.
