...
Considerem $f(x,y) = \\sqrt{x+y}$. Volem aproximar en $(1.03, 2.98)$.
Triem el punt base $P(1, 3)$ perquè $1+3=4$ és un quadrat perfecte.
Increment: $h = 0.03, \\, k = -0.02$.
...
**1. Definició de la funció i punt base:**
Considerem $f(x,y) = (xy)^{1/3}$. Aproximem en $(0.98, 1.02)$.
Punt base $P(1,1)$. Increment: $h = -0.02, \\, k = 0.02$.
...
**3. Aproximació:**
Com que $h = -k$ i $f_x = f_y$, el terme de primer grau s'anul·la: $f_x h + f_y k = 0$.
...
**1. Definició de la funció i punt base:**
Considerem $f(x,y) = x^y$. Aproximem en $(0.95, 2.01)$.
Punt base $P(1,2)$. Increment: $h = -0.05, \\, k = 0.01$.

**2. Càlcul de derivades en $P(1,2)$:**
*   $f(1,2) = 1^2 = 1$
*   $\\frac{\\partial f}{\\partial x} = y x^{y-1} \\implies \\frac{\\partial f}{\\partial x}(1,2) = 2$
*   $\\frac{\\partial f}{\\partial y} = x^y \\ln x \\implies \\frac{\\partial f}{\\partial y}(1,2) = 0$
*   $\\frac{\\partial^2 f}{\\partial x^2} = y(y-1) x^{y-2} \\implies \\frac{\\partial^2 f}{\\partial x^2}(1,2) = 2$
*   $\\frac{\\partial^2 f}{\\partial y^2} = x^y (\\ln x)^2 \\implies \\frac{\\partial^2 f}{\\partial y^2}(1,2) = 0$
*   $\\frac{\\partial^2 f}{\\partial y \\partial x} = x^{y-1} (1 + y \\ln x) \\implies \\frac{\\partial^2 f}{\\partial y \\partial x}(1,2) = 1$
