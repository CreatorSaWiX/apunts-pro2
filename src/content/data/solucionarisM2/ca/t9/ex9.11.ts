...
$$\\frac{\\partial^2 f}{\\partial x^2} = \\frac{\\partial}{\\partial x} [2x\\mu \\cos(x^2+y^2)] = 2\\mu \\cos(x^2+y^2) - 4x^2\\mu \\sin(x^2+y^2) \\implies \\frac{\\partial^2 f}{\\partial x^2}(0,0) = 2\\mu$$

$$\\frac{\\partial^2 f}{\\partial y^2} = \\frac{\\partial}{\\partial y} [2y e^{y^2} + 2y\\mu \\cos(x^2+y^2)] = 2e^{y^2} + 4y^2e^{y^2} + 2\\mu \\cos(x^2+y^2) - 4y^2\\mu \\sin(x^2+y^2) \\implies \\frac{\\partial^2 f}{\\partial y^2}(0,0) = 2 + 2\\mu$$

$$\\frac{\\partial^2 f}{\\partial y \\partial x} = 0 \\implies \\frac{\\partial^2 f}{\\partial y \\partial x}(0,0) = 0$$
...
Com que $f(0,0) = 1, \\, \\frac{\\partial f}{\\partial x}(0,0) = 0, \\, \\frac{\\partial f}{\\partial y}(0,0) = 0$:
...
Amb $\\lambda = 0$ i $\\mu = 0.2$, els valors de les segones derivades en $(0,0)$ són:
*   $\\frac{\\partial^2 f}{\\partial x^2}(0,0) = 2(0.2) = 0.4$
*   $\\frac{\\partial^2 f}{\\partial y^2}(0,0) = 2 + 2(0.2) = 2.4$
*   $\\frac{\\partial^2 f}{\\partial y \\partial x}(0,0) = 0$

La matriu Hessiana en $(0,0)$ és:
$$H(0,0) = \\begin{pmatrix} 0.4 & 0 \\\\ 0 & 2.4 \\end{pmatrix} \\implies \\Delta = (0.4)(2.4) - 0 = 0.96 > 0$$

Com que $\\Delta > 0$ i $\\frac{\\partial^2 f}{\\partial x^2} = 0.4 > 0$, el punt $(0,0)$ és un **Mínim relatiu**.`,
