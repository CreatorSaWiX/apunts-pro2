...
import dfs_code from "../code/dfs/source.cpp?raw";
...
export const dfs: Simulation = {
  ...
  generateSteps: (): SimulationStep[] => {
    ...
    updateVars().addStep(2, "algorithms.dfs.init");

    P.push(0);
    visitat[0] = true;
    W.push(0);
    builder.setHighlight(0, "#10b981");
    updateVars().addStep(6, "algorithms.dfs.addStart");

    while (P.length > 0) {
      const x = P[P.length - 1];

      // Clear current yellow highlights to green
      const currentHighlights = (builder.build().slice(-1)[0]?.visual
        ?.highlights || {}) as Record<string, string>;
      for (const key in currentHighlights) {
        if (currentHighlights[key] === "#facc15")
          builder.setHighlight(key, "#10b981");
      }

      builder.setHighlight(x, "#facc15");
      updateVars({ x: x.toString() }).addStep(11, "algorithms.dfs.checkTop", {
        x: x.toString(),
      });
      ...
      for (const y of adj[x]) {
        if (!visitat[y]) {
          ...
          updateVars({ x: x.toString(), y: y.toString() }).addStep(
            16,
            "algorithms.dfs.visitNeighbor",
      ...
      if (!hi_ha_nou) {
        updateVars().addStep(24, "algorithms.dfs.noNeighbors", {
          x: x.toString(),
        });
        const popped = P.pop();
    ...
    updateVars().addStep(29, "algorithms.dfs.finished");
