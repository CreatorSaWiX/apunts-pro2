...
import { Briefcase, BookOpen, Star, ArrowRight, ChevronLeft } from 'lucide-react';
...
const THEME_COLOR_MAP: Record<string, ThemeColorStyles> = {
    indigo: {
        bg: 'bg-indigo-500/10',
...
const DEFAULT_THEME_COLOR: ThemeColorStyles = THEME_COLOR_MAP.sky;
...
const SpecializationSidebar: React.FC<SpecializationSidebarProps> = React.memo(({
    currentSpecId,
    activeSpecId,
    mobileView,
    onSelectSpec,
        ...
        <div className={`w-full md:w-1/3 border-b md:border-b-0 md:border-r border-white/10 p-6 flex-col z-10 bg-[#020617]/40 overflow-y-auto custom-scrollbar min-h-0 ${mobileView === 'menu' ? 'flex flex-1 md:flex-none' : 'hidden md:flex'}`}>
            ...
            <div className="flex-1 flex flex-col gap-3 relative min-h-0">
                {specializations.map(spec => {
                    ...
                    const specTheme = THEME_COLOR_MAP[spec.themeColor] || DEFAULT_THEME_COLOR;

                    return (
                        <div key={spec.id} className="relative shrink-0">
                            {isPreviewed && (
                                <motion.div
                            ...
                            <button
                                ...
                                aria-label={`Seleccionar menció ${spec.name}`}
                            >
...
SpecializationSidebar.displayName = 'SpecializationSidebar';

interface SpecializationDetailsViewProps {
    activeSpec: SpecializationData;
...
const SpecializationDetailsView: React.FC<SpecializationDetailsViewProps> = React.memo(({
    activeSpec,
    theme,
    currentSpecId,
    mobileView,
    ...
    const translatedRoles = t(`curriculum.specializations.${activeSpec.id}.roles`, { returnObjects: true, defaultValue: activeSpec.roles });
    const roles: string[] = Array.isArray(translatedRoles) ? (translatedRoles as string[]) : activeSpec.roles;

    const translatedBenefits = t(`curriculum.specializations.${activeSpec.id}.benefits`, { returnObjects: true, defaultValue: activeSpec.benefits });
    const benefits: string[] = Array.isArray(translatedBenefits) ? (translatedBenefits as string[]) : activeSpec.benefits;

    const isAlreadySelected = currentSpecId === activeSpec.id;
    ...
    return (
        <div className={`w-full md:w-2/3 py-6 md:py-8 pr-6 md:pr-8 pl-4 md:pl-6 flex-col z-10 relative min-h-0 h-full ${mobileView === 'content' ? 'flex flex-1 md:flex-none' : 'hidden md:flex'}`}>
            ...
            <button
                type="button"
                onClick={onBack}
                className="md:hidden self-start mb-4 flex items-center gap-2 text-white font-bold hover:text-slate-300 transition-colors"
            >
                    ...
                    >
                        <div className="flex items-center gap-4 mb-4">
                            <h2 className="text-3xl font-black text-white tracking-tight">
                                {t(`curriculum.specializations.${activeSpec.id}.name`, activeSpec.name)}
                            </h2>
                        </div>
            ...
            <div className="pt-4 border-t border-white/10 flex justify-end shrink-0 pl-2">
                <button
                    type="button"
                    onClick={() => onConfirm(activeSpec.id)}
                    disabled={isAlreadySelected}
                    className={`px-8 py-3 rounded-xl font-bold flex items-center gap-3 transition ${
                        isAlreadySelected
                            ? 'bg-white/10 text-white cursor-default opacity-50'
                            : `bg-white text-black hover:scale-105 active:scale-95 ${theme.shadow}`
                    }`}
                >
                    {isAlreadySelected
                        ? t('planner.roadmapSpecialization.alreadySelected', 'Ja Seleccionada')
                        : t('planner.roadmapSpecialization.selectSpecialization', 'Seleccionar Especialitat')}
                    {!isAlreadySelected && <ArrowRight size={18} />}
                </button>
            </div>
        </div>
    );
});
SpecializationDetailsView.displayName = 'SpecializationDetailsView';

// --- Modal Content Inner Component ---

const SpecializationModalContent: React.FC<{
    currentSpecId: string | null;
    onClose: () => void;
    onSelect: (id: string) => void;
}> = ({ currentSpecId, onClose, onSelect }) => {
    const [previewSpecId, setPreviewSpecId] = useState<string | null>(() => currentSpecId);
    const [mobileView, setMobileView] = useState<'menu' | 'content'>('menu');

    const activeSpecId = previewSpecId || currentSpecId || specializations[0].id;
    const activeSpec = specializations.find(s => s.id === activeSpecId) || specializations[0];
    const theme = THEME_COLOR_MAP[activeSpec.themeColor] || DEFAULT_THEME_COLOR;
...
export const SpecializationModal: React.FC<SpecializationModalProps> = ({
    isOpen,
    onClose,
    currentSpecId,
    onSelect
