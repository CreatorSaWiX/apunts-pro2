...
export const Arrow = ({ memoDir, length, color, head, width }: any) => {
    ...
    useEffect(() => {
        return () => {
            // ArrowHelper creates internal cone and cylinder geometries/materials.
            if (helper.cone) {
                if (helper.cone.geometry) helper.cone.geometry.dispose();
                if (helper.cone.material) (helper.cone.material as THREE.Material).dispose();
            }
            if (helper.line) {
                if (helper.line.geometry) helper.line.geometry.dispose();
...
export const FunctionSurface = ({ f, colorScale = 5, showWireframe = false, opacity = 1 }: { f: (x: number, y: number) => number, colorScale?: number, showWireframe?: boolean, opacity?: number }) => {
    ...
    useEffect(() => {
        return () => {
            geometry.dispose(); // FIX: Awwwards critical memory leak
        };
    }, [geometry]);
...
export const Point = ({ position, color = "white" }: { position: [number, number, number], color?: string }) => (
